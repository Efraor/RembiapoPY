...
def register_local(email: str, password: str, name: str = "", role: str = "client") -> dict:
    ...
    user_id = create_user_local(email, password, name=name, role=role)
...
def login_google(id_token_str: str) -> dict:
    ...
    try:
        ...
        return {"ok": False, "error": f"Token de Google invalido (verify): {repr(e)}"}
    ...
    if aud not in allowed_client_ids:
        return {
            "ok": False,
            "error": (
                "Token de Google invalido (audience mismatch). "
                f"aud={aud} no coincide con GOOGLE_CLIENT_ID."
            ),
        }

    if iss not in ("accounts.google.com", "https://accounts.google.com"):
        return {"ok": False, "error": f"Token de Google invalido (issuer): iss={iss}"}
